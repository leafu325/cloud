...
port = 8001 
peers = [('172.17.0.6', 8001), ('172.17.0.5', 8001)]  
...
class P2PNode:
    def __init__(self, port, peers):
        ...
        self.sock.bind(('172.17.0.7', self.port)) #�~@~Y�~X��~\��~@�~^�~Z~D IP
    ...
    def _listen(self):
        while True:
            ...
            if info == 'checkAllChains':
                print(f"test1")
                hsh_code = other_chekcAllChains(self)
            ...
            elif addr != ('172.17.0.6', 8001) :
                print(f"test2")
                print(f"Hash code {info} from {addr}")
                if hsh_code != info:
                    print(f"('172.17.0.6', 8001) and {addr} -> error")
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if commands[0] == "transaction":
            ...
            checkAllChains(node,"checkAllChains")
