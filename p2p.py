...
class P2PNode:
    ...
    def _listen(self):
        ...
        while True:
            ...
            info = [data.decode('utf-8') for d in data]

            if info[0] == 'checkAllChains':
                other_chekcAllChains(self)

            elif info[0] == "transaction":
            ...
            elif info[0] == 'other_chekcAllChains':
                ...
                print(f"Hash code:{info[1]} from {addr}")
                if test_hsh_code != info[1]:
                    print(f"('172.17.0.6', 8001) or {addr} -> NO")
                else:
                    print(f"('172.17.0.6', 8001) or {addr}->Yes")
...
def checkAllChains(communicator):
    ...
    command = ['checkAllChains']
    for peer in communicator.peers:
            communicator.sock.sendto(command.encode('utf-8'), peer)
"""
...
def other_chekcAllChains(self):
    ...
    with open(volume_locate+last_block,'r') as f:
        ...
        message = ["other_chekcAllChains","hsh_code"]
